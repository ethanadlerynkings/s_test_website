/* ============================================================================
   "Silk" flow-shader backdrop — plain WebGL1, no libraries.

   Renders a single fullscreen triangle into #bg. Uniforms are packed exactly
   as the shader header specifies: seven vec4s plus u_colors[8], which keeps
   the fragment uniform count at 15 — one under WebGL1's guaranteed minimum.

   Behaviour:
     - devicePixelRatio capped at 2
     - RAF paused while the tab is hidden (clock resumes where it left off)
     - prefers-reduced-motion renders one static frame instead of animating
     - context loss is handled; the scene rebuilds on restore
     - if WebGL is unavailable the canvas is removed and the page is unaffected
   ========================================================================== */
(() => {
  'use strict';
...
void main() {
  ...
  /* --- recipe ------------------------------------------------------------ */
  // #02010A, #04052E, #3D2C8D, #916BBF — first four slots are the ones used.
  const PALETTE = [
    0.008, 0.004, 0.039,
    0.016, 0.020, 0.180,
    0.239, 0.173, 0.553,
  ...
  const TIME_SCALE = 0.76;                       // speed 35/100
  const SHAPE     = [1.26, 0.28, 0.50, 0.00];    // scale, intensity, paramA, warp
  const SURFACE   = [2.40, 1.11, 0.00, 1.00];    // detail, contrast, brightness, saturation
  const FINISH    = [0.00, 0.00, 0.000, 0.05];   // hue, vignette, blur, grain
  const TRANSFORM = [1581.0, 0.00, 0.00, 0.0];   // seed, rotation, drift, oklab
  const SPACE     = [0.00, 0.00, 0.0, 0.0];      // offset.xy, pointer.xy
  const CURSOR    = [0.0, 2.0, 0.65, 0.46];      // presence 0 => cursor off
  ...
  let raf = 0, epoch = 0, elapsed = 0;

  const giveUp = () => { canvas.remove(); };
  ...
  function build() {
    ...
    // Shader objects are reference-counted by the program; drop our handles.
    gl.deleteShader(vs);
    gl.deleteShader(fs);
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.error('[silk] link failed:', gl.getProgramInfoLog(program));
      return false;
    }
    ...
    // u_colors is an array: pad the four unused slots with the last colour so
    // no slot is ever undefined, even though u_colorCount keeps them unread.
    const colors = new Float32Array(24);
    colors.set(PALETTE);
    for (let i = 12; i < 24; i += 3) colors.set(PALETTE.slice(9, 12), i);
    gl.uniform3fv(at('u_colors[0]'), colors);
    ...
    width = height = 0;   // force the next resize to take
    return true;
  }
  ...
  function draw(seconds) {
    resize();
    gl.uniform4f(uScene, width, height, seconds * TIME_SCALE, COLOR_COUNT);
    gl.drawArrays(gl.TRIANGLES, 0, 3);
  }

  const frame = (now) => {
    raf = requestAnimationFrame(frame);
    elapsed = (now - epoch) / 1000;
    draw(elapsed);
  };

  function play() {
    if (raf || reduced.matches) return;
    epoch = performance.now() - elapsed * 1000;   // resume, don't restart
    raf = requestAnimationFrame(frame);
  }
  ...
  function start() {
    if (reduced.matches) { resize(); draw(elapsed); return; }
    play();
  }
  ...
  window.addEventListener('resize', () => {
    if (reduced.matches || !raf) { resize(); draw(elapsed); }
  }, { passive: true });
  ...
  canvas.addEventListener('webglcontextlost', (e) => {
    e.preventDefault();       // required, or the context will not be restored
    pause();
  });

  canvas.addEventListener('webglcontextrestored', () => {
    if (build()) start(); else giveUp();
  });
})();
